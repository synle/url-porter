---
name: release-beta
description: Trigger the beta release workflow for URL Porter. Creates a draft prerelease from a specific SHA.
user_invocable: true
---

# Release Beta

Trigger the beta release workflow (`release-beta.yml`) for URL Porter. Creates a draft prerelease tagged `release-beta-<date>-<sha>` with the manifest name set to "URL Porter (Beta)".
...
3. Trigger the workflow:
   - `gh workflow run release-beta.yml -f sha="<sha>" -f notes="<user notes>"`
   - Omit `-f sha` or pass empty string to use HEAD.
   - Omit `-f notes` or pass empty string for no notes.
